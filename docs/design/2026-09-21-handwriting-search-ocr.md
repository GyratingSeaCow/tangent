...
| **CPU-first, GPU as the accuracy upgrade** | "Some people will not be able to run GPUs." CPU flavour uses `microsoft/trocr-base-handwritten`; the RTX flavour uses `microsoft/trocr-large-handwritten` (better on messy cursive; near-instant on Jeff's RTX 5070). |
